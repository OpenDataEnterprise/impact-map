...
$(document).ready(function() {
  ...
  function getRegionAbbr (regionObj) {
    let regionAbbr = getNestedValue(regionObj, ['abbreviation']);
  ...
  $.ajax({
    ...
    success: function(results) {
      $.each(results, function(index, result) {
        ...
        let url = getNestedValue(result, ['url']);
        let country = getNestedValue(result,
          ['country', 'name']);
        let sector = getNestedValue(result,
          ['sector', 'sector']);
        let impact = getNestedValue(result, ['impact']);
        ...
        $("#templateAbc")
          .clone(true)
          .removeAttr('id')
          .attr('id', underscoreTitle)
          .appendTo(".abc-case-container");
        ...
        if (sector && sector in sectorContainerMap) {
          let sectorContainerName = sectorContainerMap[sector];
        ...
        if (machineReadable === 'Yes') {
          let mrContainerName = mrContainerMap[sector];
      ...
      // fill address bar with the URL for this use case using the same rule as
      // in index.php
      $('.read-more-button-1').on('click', function() {
        $(this).parent().find('div.modal-wrapper').show();
        let caseId = $(this).parent().attr('id');
      ...
      // show/hide four view containers and color interactions for four new
      // filter buttons
      // first - alphabetical
